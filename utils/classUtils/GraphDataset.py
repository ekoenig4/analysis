...
class ScaleEdgeAttrs:
    def __init__(self, invert=False):
        if not invert:
            self.transform = lambda x: (x-self.minim)/(self.maxim-self.minim)
        else:
            self.transform = lambda x: (self.maxim-x)/(self.maxim-self.minim)

    def fit(self, edge_attrs):
        self.minim = ak.min(edge_attrs)
        self.maxim = ak.max(edge_attrs)
        return self
...
def get_edge_attrs(jets, attrs=["dr"]):
    return calc_dr(jets.eta[:, :, None], jets.phi[:, :, None], jets.eta[:, None], jets.phi[:, None])[:, :, :, None]
...
def build_features(tree, node_attr_names=["m", "pt", "eta", "phi", "btag"], edge_attr_names=["dr"]):
    ...
    edge_scaler = ScaleEdgeAttrs(invert=True).fit(edge_attrs)
    ...
    return node_features, edge_features, node_class_weights, edge_class_weights, node_attr_names, edge_attr_names
...
class Dataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        return ['graphs.pt', 'attr_names.pt', 'class_weights.pt','filelist.pt']

    def process(self):
        # Read data into huge `Data` list.
        filelist = list(map(lambda f:f.fname,self.processed_tree.filelist))
        node_features, edge_features, node_class_weights, edge_class_weights, node_attr_names, edge_attr_names = build_features(
            self.processed_tree)
