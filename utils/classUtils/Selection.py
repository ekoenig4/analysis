...
class Selection(Tree):
    def __init__(self, tree, cuts={}, include=None, previous=None, variable=None, njets=-1, mask=None, tag=None, ignore_tag=False):
        ...
        self.previous_selected = previous.total_jets_selected if previous else (
            self.all_jets_mask == False)
    ...
    def choose_jets(self, cuts={}, variable=None, njets=-1, mask=None, tag=None):
        ...
        self.jets_failed = exclude_jets(
            self.tree.all_jets_mask, self.jets_passed)
    ...
    def sort_jets(self, variable, njets=-1, method=max):
        ...
        if variable is None and self.include:
            ...
            self.jets_passed_index = sort_jet_index(
                self.tree, self.variable, self.jets_passed, method=method)
    ...
    def select_njets(self, njets):
        ...
        self.njets = njets
        self.jets_selected_index, self.jets_remaining_index = get_top_njet_index(
            self.tree, self.jets_passed_index, self.njets)

        self.jets_selected = get_jet_index_mask(
            self.tree, self.jets_selected_index)
        ...
        if self.previous:
            self.total_jets_selected_index = ak.concatenate(
                [self.previous_index, self.total_jets_selected_index], axis=-1)
        self.total_jets_selected = self.previous_selected | self.jets_selected
        self.total_njets = self.previous_njets + \
            (self.njets if self.njets != -1 else 6)

        if self.is_signal:
            self.build_extra_collections()
    ...
    def sort_selected_jets(self, variable):
        self.variable = variable
        self.jets_selected_index = sort_jet_index(
            self.tree, self.variable, self.jets_selected)
    ...
    def reco_X(self):
        def fill_zero(ary): return ak.fill_none(
            ak.pad_none(ary, 6, axis=-1, clip=1), 0)
        ...
        ijet_p4 = [vector.obj(pt=jet_pt[:, ijet], eta=jet_eta[:, ijet],
                              phi=jet_phi[:, ijet], mass=jet_m[:, ijet]) for ijet in range(6)]
        X_reco = ijet_p4[0]+ijet_p4[1]+ijet_p4[2] + \
            ijet_p4[3]+ijet_p4[4]+ijet_p4[5]
    ...
    def build_extra_collections(self):
        ...
        def build_extra_jet(tag, jet_mask):
            ...
            setattr(self, f"n{tag}_passed", ak.sum(
                getattr(self, f"{tag}_passed"), axis=-1))

            setattr(self, f"{tag}_passed_position", get_jet_position(
                self.jets_passed_index, jet_mask))
            setattr(self, f"{tag}_passed_index", self.jets_passed_index[getattr(
                self, f"{tag}_passed_position")])

            setattr(self, f"{tag}_failed", self.jets_failed & jet_mask)
            setattr(self, f"n{tag}_failed", ak.sum(
                getattr(self, f"{tag}_failed"), axis=-1))

            setattr(self, f"{tag}_selected", self.jets_selected & jet_mask)
            setattr(self, f"n{tag}_selected", ak.sum(
                getattr(self, f"{tag}_selected"), axis=-1))

            setattr(self, f"{tag}_selected_position", get_jet_position(
                self.jets_selected_index, jet_mask))
            setattr(self, f"{tag}_selected_index", self.jets_selected_index[getattr(
                self, f"{tag}_selected_position")])
    ...
    def title(self, i=0):
        def ignore(tag): return any(
            _ in tag for _ in ["baseline", "preselection"])
        if self.tag is None:
            return
        title = f"{self.njets} {self.tag}" if self.njets != - \
            1 else f"all {self.tag}"
        variable = self.variable if self.variable else "jet_pt"
        if variable != "jet_pt":
            title = f"{title} / {variable.replace('jet_','')}"
...
class MergedSelection(CopySelection):
    def __init__(self, selection, tag="merged selection"):
        ...
        self.jets_passed_index = ak.concatenate(
            (self.jets_selected_index, self.jets_remaining_index), axis=-1)
        self.njets_passed = ak.sum(self.jets_passed, axis=-1)
        self.njets_selected = ak.sum(self.jets_selected, axis=-1)
        self.jets_failed = exclude_jets(
            self.tree.all_jets_mask, self.jets_passed)
    ...
    def add(self, selection):
        ...
        for key in ("jets_selected_index",):
            setattr(self, key, ak.concatenate(
                (getattr(selection, key), getattr(self, key)), axis=-1))
