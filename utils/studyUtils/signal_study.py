...
class SignalStudy(Study):
    def __init__(self, selection, subset="selected", plot=True, **kwargs):
        ...
        if self.title is None:
            self.title = self.selection.title()
        self.subset = subset
...
def signal_order(selection, plot=True, saveas=None, **kwargs):
    ...
    for ijet in range(njets):
        ...
        isixb_mask = get_jet_index_mask(
            tree, sixb_ordered[:, ijet][:, np.newaxis])
        ...
        for i, (var, info) in enumerate(varinfo.items()):
            ...
            datalist = (sixb_data,)
            hist_multi(datalist, labels=labels, figax=(
                fig, axs[i % ncols]), **ord_info)
        ...
        plt.show()
        if saveas:
            save_fig(fig, "order", f"{ordinal(ijet+1)}_{saveas}")
...
def selection(
    ...
    **kwargs,
    ...
    for ijet in ijet_cut:
        ...
        if scaled:
            selection_purity *= minjet / 6

        selection_purities = np.append(selection_purities, selection_purity)
        selection_efficiencies = np.append(
            selection_efficiencies, selection_efficiency)
    ...
    graph_simple(
        ijet_cut,
        selection_purities,
        xlabel="N Jets Selected",
        ylabel=ylabel,
        label="nSixb == min(6,nSelected) / Total Events",
        figax=(fig, ax0),
    )
    ...
    if extra and required:
        graph_simple(
            ...
            xlabel="N Jets Selected",
            ylabel="Efficiency",
            label="nJets >= min(6,nSelected) / Total Events",
            figax=(fig, ax1),
        )
...
def selection_comparison(
    ...
    **kwargs,
    ...
    selection_purities = []
    if labels is None:
        labels = [selection.tag for selection in selections]
...
def jets(*args, varlist=["jet_pt", "jet_eta", "jet_btag", "jet_qgl"], **kwargs):
    study = SignalStudy(
        *args,
        varlist=varlist,
        labels=("All Jets", f"Background Jets", f"Signal Jets"),
        s_colors=("blue", "black", "tab:orange"),
        **kwargs,
    )
    ...
    for i, (var, info) in enumerate(study.varinfo.items()):
        ...
        hist_multi(
            (jets_var, bkgs_var, sixb_var), figax=(fig, axs[i]), **info, **study.attrs
        )
    ...
    plt.show()
    if study.saveas:
        save_fig(fig, f"jets_{study.subset}", study.saveas)
...
def jets_2d(selection, log=True, **kwargs):
    ...
    hist2d_simple(
        ...
        ybins=btaginfo["bins"],
        xbins=ptinfo["bins"],
        ylabel=btaginfo["xlabel"],
        xlabel=ptinfo["xlabel"],
        title=labels[0],
...
def ijets(
    selection, varlist=["jet_pt", "jet_btag", "jet_eta", "jet_phi"], njets=6, **kwargs
):
    study = SignalStudy(
        selection, varlist=varlist, s_colors=("blue", "black", "tab:orange"), **kwargs
    )
    ...
    for ijet in njets:
        ...
        ijet_mask = get_jet_index_mask(
            jets_mask, jets_ordered[:, ijet][:, np.newaxis])
...
def jets_ordered(
    *args,
    varlist=["jet_pt", "jet_eta", "jet_btag", "jet_qgl"],
    njets=1,
    topbkg=True,
    **kwargs,
):
    ...
    for ijet in range(njets):
        ...
        plt.show()
        if study.saveas:
            save_fig(fig, f"jets_{study.subset}",
                     f"{ordinal(ijet+1)}_{study.saveas}")
    ...
    for ijet in range(njets):
        ...
        if not (compare and topbkg):
            ...
            plot2d(
                ...
                xbins=ptinfo["bins"],
                ybins=ptinfo["bins"],
                xlabel=f"{ordinal(ijet+1)} Signal {ptinfo['xlabel']}",
                ylabel=f"Top Background {ptinfo['xlabel']}",
                title=f"Comparison Signal Bias: {pt_bias:0.2f}",
                density=density,
                log=log,
                figax=(fig, axs[0]),
            )
            plot2d(
                ...
                xbins=btaginfo["bins"],
                ybins=btaginfo["bins"],
                xlabel=f"{ordinal(ijet+1)} Signal {btaginfo['xlabel']}",
                ylabel=f"Top Background {btaginfo['xlabel']}",
                title=f"Comparison Signal Bias: {btag_bias:0.2f}",
                density=density,
                log=log,
                figax=(fig, axs[1]),
            )
        ...
        plt.show()
        if saveas:
            save_fig(fig, f"jets_2d_{subset}", f"{ordinal(ijet+1)}_{saveas}")
...
def x_reco(selection, plot=True, saveas=None, scaled=False, **kwargs):
    ...
    for i, (var, info) in enumerate(varinfo.items()):
        ...
        hist_multi(
            ...
            histtypes=histtypes,
            colors=colors,
            figax=(fig, axs[int(i / ncols), i % ncols]),
            **info,
        )
    fig.suptitle(title)
    fig.tight_layout()
...
def njet(selection, plot=True, saveas=None, density=0, **kwargs):
    ...
    hist_multi(
        [njets],
        bins=range(16),
        xlabel=f"Number of Jets {subset.capitalize()}",
        figax=(fig, axs[0]),
        **study.attrs,
    )
    hist_multi(
        [nsixb],
        bins=range(8),
        xlabel=f"Number of Signal Jets {subset.capitalize()}",
        figax=(fig, axs[1]),
        **study.attrs,
    )
    hist_multi(
        [nbkgs],
        bins=range(8),
        xlabel=f"Number of Background Jets {subset.capitalize()}",
        figax=(fig, axs[2]),
        **study.attrs,
    )
    ...
    plt.show()
    if saveas:
        save_fig(fig, f"njets_{subset}", saveas)
...
def presel(selection, plot=True, saveas=None, density=0, **kwargs):
    ...
    for i, (var, info) in enumerate(varinfo.items()):
        ...
        labels = (
            f"All Jets {subset.capitalize()}",
            f"Signal Jets {subset.capitalize()}",
        )
        hist_multi(datalist, labels=labels, figax=(
            fig, axs[i % ncols]), **info)
    fig.suptitle(title)
    fig.tight_layout()
    plt.show()
    if saveas:
        save_fig(fig, f"presel_{subset}", saveas)
...
def jet_issue(selection, plot=True, saveas=None, density=0, **kwargs):
    ...
    hist_multi([jet_eta], bins=etabins,
               xlabel="jet eta", figax=(fig, axs[0, 1]))
...
def jet_comp(selection, plot=True, saveas=None, signal=False, density=0, **kwargs):
    ...
    signal_index = {
        tag: get_jet_index_mask(tree, tree[f"gen_{tag}_recojet_index"][:, np.newaxis])[
            mask
        ]
    ...
    signal_b_selected = {tag: jets & b_index for tag,
                         b_index in signal_index.items()}

    nrows, ncols = 1, 1
    fig0, ax0 = plt.subplots(nrows=nrows, ncols=ncols, figsize=(16, 2.5))
    nsignal_b_selected = np.array(
        [
            ak.sum(ak.sum(signal_mask, axis=-1))
            for signal_mask in signal_b_selected.values()
        ]
    )
    graph_simple(
        signal_tags, nsignal_b_selected, ylabel="Number Jets", figax=(fig0, ax0)
    )
...
def compare_scores(
    ...
    **kwargs,
    ...
    valuemap = {
        value: np.array([getattr(score, value) for score in scores]) for value in values
    }
    if prod and len(values) > 1:
        prodkey = "*".join(valuemap.keys())
        for scorelist in list(valuemap.values()):
            if prodkey not in valuemap:
                valuemap[prodkey] = scorelist
            else:
                valuemap[prodkey] = valuemap[prodkey] * scorelist
