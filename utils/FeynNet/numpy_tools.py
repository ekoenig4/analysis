...
def group_products(product_assignment : dict, **features):
    ...
    grouped_features = np.concatenate([ features[particle_type][:, :, permutations] for particle_type, permutations in product_assignment.items() ], axis=-1)
...
def aggregate_products(product_assignment : dict, aggr='max', **features):
    ...
    if aggr == 'max': return grouped_features.max(axis=3)